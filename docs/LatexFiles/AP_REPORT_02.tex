...
This project report discusses the development and implementation of a Maths Interpreter Software, aimed at evaluating and plotting maths expression. The main objective was to create an application capable of evaluating complex maths expression, with focus on performing arithmetic operations, plotting lines and polynomials, and assigning variables. Our methodology included Model-View-ViewModel, Dependency Injection and Service Layer design patterns, and followed best practices such as SOLID principles. The development started with creating core functionality like the F\# interpreter and C\# GUI. As we progressed through the semester, we implemented compulsory features like expression solving, plotting and variable assignment that were outlined in the MoSCoW table. In the end, we managed to complete not just all 'Must' and 'Should' objectives, but also several 'Could' features, such as adding tangents, differentiation, visualising parse tree and area under the curve, finding roots, functions, for-loop, which made our app more versatile. Overall, this project not only met all of it compulsory objectives but also included several optional features. The result is a user-friendly, versatile and robust Maths Interpreter Software that proves the effectiveness of our methodology. The report also discusses potential future improvements, such as support for Complex and Rational numbers and Parser optimisation, which would improve software's versatility and performance.
